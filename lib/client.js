...
class Client extends EventEmitter {
  ...
  constructor() {
    ...
    // used for reconstructing the client from a forkingworker
    this.arguments = arguments;

    // expose qless messages for worker management and process subscribers
    this.sub.psubscribe('ql:*');
    this.sub.on('pmessage', (p, c, m) => {
      try {
        this.emit('message', { channel: c, message: JSON.parse(m) });
      } catch (e) {
        // @TODO fix track publish to encode like everything else
        this.emit('message', { channel: c, message: m });
      }
    });
  }
