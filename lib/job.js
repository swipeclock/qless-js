...
class BaseJob {
  ...
  getKlass(options) {
    ...
    return BaseJob.importKlass(name);
  }
...
class Job extends BaseJob {
  ...
  fail(group, message, cb) {
    this.logger.info(`Failing Job ${this.jid}`);
  ...
  retry(cb, group, message, delay) {
    ...
    this.logger.info(`Retrying job ${this.jid} in ${delay} seconds`);
