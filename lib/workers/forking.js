...
class ForkingWorker extends Worker {
  ...
  spawn(cb) {
    ...
    const child = cp.fork(`${__dirname}/serialEntry`, args, { env: { DEBUG: '*' } }, __dirname);
    this.sandboxes[child.pid] = child;
    this.logger.info(`Started worker pid: ${child.pid}`);

    child.on('error', (err) => {
      this.logger.error(`Error in child process ${child.pid}: ${err}`);
      if (cb) cb(err);
    });

    child.on('exit', (code, signal) => {
      this.logger.info(`Child process:${child.pid} has closed with code:${code} and signal:${signal}`);
