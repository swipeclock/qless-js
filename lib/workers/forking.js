...
class ForkingWorker extends Worker {
  ...
  _createWatchdog(workerName, jid, expiresAt) {
    ...
    this.logger.debug(`create watchdog for worker:${workerName} with jid:${jid} with ttl:${ttl}`, { jid });
    this.watchdogs[workerName] = setTimeout((
      ...
      if (this.jobLedger[workerName] !== jid) {
        ...
        this.logger.error(`watchdog jid mismatch!  watchdogJid:${jid} ledgerJid:${this.jobLedger[workerName]}`, { jid });
        return;
      }
