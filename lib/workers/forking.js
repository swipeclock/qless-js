...
class ForkingWorker extends Worker {
  ...
  _createWorkerSub(workerName) {
    ...
    this.logger.info(`subsribing to worker channel:${channel}`);
  ...
  _removeWorkerSub(workerName) {
    const channel = `ql:w:${workerName}`;
    this.logger.info(`unsubsribing from worker channel:${channel}`);
  ...
  _processPubSub(channel, message) {
    ...
    switch (message.event) {
      case 'heartbeat':
        this.logger.debug(`received heartbeat for worker:${message.worker} jid:${message.jid}`);
        this._removeWatchDog(message.worker);
        this._createWatchdog(message.worker, message.jid, message.expires);
        break;
      case 'lock_lost':
      case 'canceled':
        this.logger.info(`canceled or lock_lost for worker:${message.worker} jid:${message.jid}`);
        this._killWorker(message.worker, true);
        break;
      default:
  ...
  stop(force = false) {
    ...
    Object.keys(this.sandboxes).forEach((workerName) => {
      this._killWorker(workerName, force);
    });
   ...
   * @private
   */
  _killWorker(workerName, force = false) {
    const sig = force ? 'SIGTERM' : 'SIGINT';
    this.logger.info(`Sending kill to worker:${workerName} with signal:${sig}`);
    this.sandboxes[workerName].kill(sig);
  }
  ...
  _spawn(child, cb) {
    ...
    this.logger.info(`Started worker: ${workerName}`);
    ...
    child.on('message', (jobInfo) => {
      this.logger.debug(`child worker:${workerName} is working on ${jobInfo.jid}`);
  ...
  _createWatchdog(workerName, jid, expiresAt) {
    ...
    this.logger.debug(`create watchdog for worker:${workerName} with jid:${jid} with ttl:${ttl}`);
    this.watchdogs[workerName] = setTimeout((
    ) => {
      console.log(`watchdog barking for worker:${workerName} with jid:${jid}`);
      if (this.jobLedger[workerName] !== jid) {
        // This should never happen
        this.logger.error(`watchdog jid mismatch!  watchdogJid:${jid} ledgerJid:${this.jobLedger[workerName]}`);
        return;
      }

      this._killWorker(workerName, true);
    }, ttl);
  }
  ...
  static _getTTL(expiresAt) {
    const expired = new Date(expiresAt * 1000);
    const diff = expired.getTime() - new Date().getTime();

    return diff + ForkingWorker._getGracePeriod();
  }
